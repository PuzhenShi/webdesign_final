...
function VideoCover(props) {
  const { title, duration, views, cover, author, id, date, author_id } =
    props.videoInfo;
  const url =
  ...
  return (
    <div style={wrapperStyle} className="video-display">
      <div style={contentStyle} className="cover">
        ...
        <Link to="video?id={id}" style={titleStyle} className="title">
          {title}
        </Link>
        <br></br>
        <Link to="user?id={author_id}" style={titleStyle} className="title">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
            height="16"
